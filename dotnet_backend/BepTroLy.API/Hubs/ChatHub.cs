...
public class ChatHub : Hub
{
    ...
    public async Task SendMessage(int fridgeId, string content)
    {
        try
        {
            ...
            // Validate fridge membership
            var isMember = await _db.FridgeMembers
                .AnyAsync(m => m.FridgeId == fridgeId && m.UserId == userId && m.Status == "accepted");

            if (!isMember)
            {
                Console.WriteLine($"[CHAT] SendMessage failed: User {userId} is not a member of fridge {fridgeId}");
                return;
            }
